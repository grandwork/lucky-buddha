        ...
        for (var r=0; r < round_count; r++) {
          ...
          for (var g=0; g < game_count; g++) {
            ...
            for (var t=0; t < team_count; t++) {
			  ...
			  $(".g_team h3").unbind("click");
			  $(".g_team h3").click(function(){
				  var gseed=$(this).children(".g_seed").text();
				  var gname=$(this).text().substring($(this).text().indexOf(" ")+1,$(this).text().length);
				  var classlist = $(this).parent('.g_team').attr('class').split(/\s+/);
				  var gid=classlist[1];
				  var gwinner={"name" : gname,"id" : gid, "seed" : gseed};
				  //console.log($(this).children(".g_seed").text());
				  //console.log($(this).text().substring($(this).text().indexOf(" ")+1,$(this).text().length));
				  //console.log(classlist[1]);
				  var ggame=$(this).parent().parent().index();
				  var labels=$(".g_gracket .g_round_label").length;
				  var ground=$(this).parent().parent().parent().index()-labels;
				  if(ground!==1) ggame=(ggame-1)/2;
				  //console.log(ground);
				  //console.log(Math.round(ggame/2));
				  var updated=new Array();
				  updated=data;
				  if(typeof updated[ground]== "undefined") updated[ground]=new Array();
				  //console.log(updated[0][Math.round(ggame/2)][ggame%2]);
				  //if(typeof updated[ground][Math.round(ggame/2)] === "undefined") updated[ground][Math.round(ggame/2)]=new Array();
				  //console.log(updated[ground][Math.floor(ggame/2)]);
				  if(typeof updated[ground][Math.floor(ggame/2)] == "undefined") updated[ground][Math.floor(ggame/2)]=new Array();
				  updated[ground][Math.floor(ggame/2)][ggame%2]=gwinner;
				  //console.log(updated[ground][Math.floor(ggame/2)]);
				  console.log(ground);
				  while(typeof updated[ground+1] !="undefined"){
					updated[ground+1]=new Array();
					ground++;
				  }
				  console.log(updated);
				  $(".my_gracket").html('');
				  //console.log(updated);
				  $(".my_gracket").gracket({ src : updated});
			  })
            };
